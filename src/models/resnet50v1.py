...
def residual_block(input_tensor: tf.Tensor, filters: Tuple[int, int], s: int = 1,
    ...
    # Block 1
    x = tf.keras.layers.Conv2D(f1, kernel_size=(1, 1), strides=(1, 1), kernel_regularizer=REGULARIZER)(input_tensor)
...
def stack_blocks(x: tf.Tensor, filters: Tuple[int, int], num_blocks: int, s: int = 2) -> tf.Tensor:
    x = residual_block(x, filters, s, conv_shortcut=True)
    for _ in range(2, num_blocks + 1):
        x = residual_block(x, filters)
    return x


class ResNet50v1(BaseModule):
